...
public class JSR303Aspect {
    ...
    public Object doAround(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        for (Annotation[] ants : argAnnotations) {
            if (ants.length > 0 && (ants[0].annotationType().getSimpleName().equals("NotBlank") || ants[0].annotationType().getSimpleName().equals("NotNull"))) {
    ...
    protected boolean writeResult(BindingResult result) {
        if (result.hasErrors()) {
            ...
            if (null != fieldErrors && fieldErrors.size() > 0) {
                ...
                rs.setCode(2);
                rs.setMsg("参数检验不通过[" + fieldErrors.get(0).getDefaultMessage() + "]");
                rs.setData(null);
